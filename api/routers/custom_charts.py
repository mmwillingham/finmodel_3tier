...
def _normalize_selected_account_ids(selected_account_ids):
    if isinstance(selected_account_ids, str):
        try:
            return json.loads(selected_account_ids) if selected_account_ids else []
        except json.JSONDecodeError:
            return []
    return selected_account_ids or []


def expand_series_configurations(series_configs, db: Session, current_user: models.User) -> List[dict]:
    ...
    for series_config in series_configs:
        ...
        if itemize and item_id is None:
            items = []
            item_name_field = 'name' if item_type in ['assets', 'liabilities'] else 'description'
            ...
            if items:
                for idx, item in enumerate(items):
                    ...
                    new_series['itemize'] = False
                    new_series['label'] = getattr(item, item_name_field, str(item.id))
    ...
    return expanded_series_configs


def build_projection_accounts(series_configs, db: Session, current_user: models.User):
    expanded_series_configs = expand_series_configurations(series_configs, db, current_user)
        ...
        if item_type and item_id:
            ...
            if item_type == 'assets':
                ...
                for item in query.all():
                    ...
                    if item.linked_item_type == "asset" and item.percentage is not None:
                        ...
                        for asset in linked_assets:
                            linked_asset_ids_needed.add(asset.id)
                    else:
                    ...
                    base_name = account_name.split("|LINKED:")[0] if "|LINKED:" in account_name else account_name
                ...
                for item in query.all():
                    ...
                    if "|LINKED:" in account_name:
                        base_name = account_name.split("|LINKED:")[0]
    ...
    included_income_names = {
        acc.name.split("|LINKED:")[0] if "|LINKED:" in acc.name else acc.name
        for acc in accounts_for_projection if acc.account_type == "income"
    }
    included_expense_names = {
        acc.name.split("|LINKED:")[0] if "|LINKED:" in acc.name else acc.name
        ...
        if item.description not in included_income_names:
            ...
            if item.linked_item_type == "asset" and item.percentage is not None:
                ...
                if linked_assets:
                    ...
                    account_name = f"{item.description}|LINKED:{','.join(asset_names)}|PERCENTAGE:{item.percentage}"
                    for asset in linked_assets:
                        if asset.name not in added_account_names:
                            accounts_for_projection.append(schemas.ProjectedAccountCreate(
                                name=asset.name,
                                account_type='asset',
                                initial_value=asset.value,
                                contribution=0.0,
                                growth_rate=asset.annual_increase_percent,
                                loan_type=None, principal_amount=None, interest_rate=None, loan_term_months=None, loan_start_date=None, monthly_payment=None,
                                start_date=asset.start_date, end_date=asset.end_date
                            ))
                            added_account_names.add(asset.name)
            else:
                contribution = item.yearly_value / 12
    ...
    for item in all_expense_items:
        if item.description not in included_expense_names:
            ...
            if "|LINKED_INCOME:" in account_name:
                included_expense_names.add(account_name.split("|LINKED_INCOME:")[0])

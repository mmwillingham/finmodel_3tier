...
import traceback
import sys
print(f"--- DEBUG: api/routers/custom_charts.py LOADED (Traceback: {traceback.format_exc()}) ---"); sys.stdout.flush()
...
def fetch_and_convert_item(db: Session, current_user: models.User, item_type: str, item_id: int) -> Optional[schemas.ProjectedAccountCreate]:
    print(f"""--- DEBUG: Attempting to fetch item_type: {item_type}, item_id: {item_id} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
    if item_type == 'assets':
        item = db.query(models.Asset).filter(models.Asset.id == item_id, models.Asset.owner_id == current_user.id).first()
        if item:
            print(f"""--- DEBUG: Found asset: {item.name} (ID: {item.id}, Value: {item.value}) (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
            ...
            print(f"""--- WARNING: Asset with ID {item_id} not found for user {current_user.id} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
        ...
        if item:
            print(f"""--- DEBUG: Found liability: {item.name} (ID: {item.id}, Value: {item.value}) (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
            return schemas.ProjectedAccountCreate(
                name=item.name,
            ...
            print(f"""--- WARNING: Liability with ID {item_id} not found for user {current_user.id} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
        ...
        if item:
            print(f"""--- DEBUG: Found cashflow item: {item.description} (ID: {item.id}, Yearly Value: {item.yearly_value}, Is Dynamic: {bool(item.linked_item_id)}) (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
            ...
            print(f"""--- WARNING: CashFlowItem with ID {item_id} not found for user {current_user.id} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
    return None
...
def create_custom_chart(
    ...
    try:
        print(f"""--- DEBUG: TOP OF create_custom_chart function. User ID: {current_user.id} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
        print(f"""--- DEBUG: Entering create_custom_chart for user {current_user.id} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
        ...
        print(f"""--- DEBUG: Parsed series configurations: {series_configs} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
        print(f"""--- DEBUG: Projection years from user settings: {projection_years} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
        ...
        for series_config in series_configs:
            ...
            if item_type and item_id: # Specific item selected
                ...
                if account:
                    ...
                    print(f"""--- WARNING: Could not find item {item_id} of type {item_type} for user {current_user.id} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
        ...
        print(f"""--- DEBUG: Accounts prepared for projection (after loop): {json.dumps([acc.model_dump() for acc in accounts_for_projection], indent=2)} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush() # NEW DEBUG LINE
        print(f"""--- DEBUG: Attempting to call calculate_projection for chart {chart.name} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush() # NEW DEBUG LINE
        ...
        print(f"""--- DEBUG: Projection calculation successful. Final Value: {projection_results['final_value']} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
        print(f"""--- DEBUG: data_json content after calculation, before model assignment: {projection_results.get('data_json')} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush() # NEW DEBUG LINE
        print(f"""--- DEBUG: data_json content before saving in create_custom_chart: {projection_results.get('data_json')} (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush() # Debug log for create
        ...
        db.commit()
        db.refresh(db_chart)
        print(f"""--- DEBUG: Custom chart {db_chart.name} created with ID {db_chart.id} and projection results. (Traceback: {traceback.format_exc()}) ---"""); sys.stdout.flush()
        return db_chart

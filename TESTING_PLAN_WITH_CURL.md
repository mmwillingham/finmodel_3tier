...
ASSET_2_4_ID=$(curl -s -X POST "${API_BASE}/assets/" \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Test Asset 2.4",
...
INCOME_2_4_ID=$(curl -s -X POST "${API_BASE}/cashflow?is_income=true" \
  ...
  -d "{
    ...
    \"percentage\": null,
    \"person\": \"Family\",
    \"contributes_to_asset_id\": ${ASSET_2_4_ID}
  }" | jq -r '.id')

echo "Created Asset ID: $ASSET_2_4_ID, Income ID: $INCOME_2_4_ID"
```

**Expected Values:**
- Year 1: Asset = $100,000.00 (from income contribution)
- Year 2: Asset = $200,000.00 (100,000 + 100,000)
- Year 3: Asset = $300,000.00 (200,000 + 100,000)
...
curl -s -X DELETE "${API_BASE}/cashflow/${INCOME_2_4_ID}" \
  -H "Authorization: Bearer ${TOKEN}"
curl -s -X DELETE "${API_BASE}/assets/${ASSET_2_4_ID}" \
  -H "Authorization: Bearer ${TOKEN}"
echo "Deleted Income ID: $INCOME_2_4_ID, Asset ID: $ASSET_2_4_ID"
```
...
**Setup:**
```bash
# Create 3 expenses in "Investment" category

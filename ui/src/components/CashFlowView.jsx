...
export default function CashFlowView({ type, incomeItems, expenseItems, refreshCashflow, validCategories = [], assets = [], autoDisbursements = [] }) {
  ...
  const displayedItems = type === 'income' ? [...items, ...derivedIncomeItems] : items;
  ...
  const [stateTaxValue, setStateTaxValue] = useState(null);
  ...
  const total = displayedItems.reduce((sum, item) => sum + (item.yearly_value || 0), 0);

  // Count items with missing fields and auto-generated items
  const itemStats = displayedItems.reduce((stats, item) => {
  ...
  const handleDownloadCashFlowTableCsv = (filename) => {
    if (displayedItems.length > 0) {
      ...
      const formattedData = displayedItems.map(item => {
        const row = {
                ...
                {[
                  ...
                  <td key="actions" className="action-buttons-cell">
                    {!item.syntheticTaxableDistribution ? (
                      ...
                      <span style={{ fontStyle: 'italic', color: '#555' }}>Auto-generated</span>
                    )}
                  </td>,
                ]}
              </tr>

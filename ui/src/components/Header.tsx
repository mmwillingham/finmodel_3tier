...
const initialContactModalState: ContactModalState = {
  isOpen: false,
  contactType: '',
  label: '',
};
...
const Header: React.FC = () => {
  ...
  const [contactModal, setContactModal] = useState<ContactModalState>(initialContactModalState);
  ...
  const settingsDropdownRef = useRef<HTMLDivElement | null>(null);
    ...
    useEffect(() => {
        ...
        const handleClickOutside = (event: MouseEvent) => {
            if (settingsDropdownRef.current && event.target instanceof Node && !settingsDropdownRef.current.contains(event.target)) {
    ...
    const toggleContactMenu = () => {
        setShowContactMenu((prev: any) => !prev);
    };

    const openContactModal = (contactType: string, label: string) => {
    ...
    const handleTourSettingsMenuRequest = (shouldOpen: boolean) => {
    ...
    return (
        <header className="app-header">
            <nav>
                ...
                <div className="logo">
                    <Link to={currentUser ? "/app" : "/"} onClick={handleLogoClick}>
                        {/* 
                        <img src="/vault-logo.jpg" alt="" style={{ height: '32px', verticalAlign: 'middle', marginRight: '8px' }} onError={(e: any) => { e.target.src = '/vault-logo.png'; }} />
                        */}                        
            ...
            <ContactFormModal
                ...
                onClose={() => setContactModal(initialContactModalState)}
            />
            <TourModal

...
export default function SidebarLayout() {
  ...
  const projectedNetWorth = useMemo(() => {
    ...
    const assetFuture = assets.reduce((sum, asset) => {
    ...
    const liabilityFuture = liabilities.reduce((sum, liability) => {
  ...
  const individualAssetAllocationData = useMemo(() => {
    ...
    const remainingTotal = sortedAssets.slice(topCount).reduce((sum, asset) => sum + asset.value, 0);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setDashboardSimulation((prev) => ({ ...prev, loading: true }));
    const runs = 300;
    setTimeout(() => {
      ...
      const successRate = (terminalValues.filter((v) => v > 0).length / runs) * 100;
      const confidenceAboveTarget = (terminalValues.filter((v) => v > (Number(desiredNetWorthTarget) || 0)).length / runs) * 100;
      if (!cancelled) {

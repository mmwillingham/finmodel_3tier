...
interface ProjectionAccountPayload {
  name: string;
  account_type: string;
...
interface BalanceSheetProjectionProps {
...
export default function BalanceSheetProjection({ assets, liabilities, incomeItems, expenseItems, projectionYears, formatCurrency, showChartTotals, compact = false, showProjectionYearSelector = false, onProjectionYearsChange, maxProjectionYears, isLimitedPlan = false }: BalanceSheetProjectionProps) {
  const { userSettings, currentUser } = useAuth();
  const currentYear = new Date().getFullYear();
  const overallChartRef = useRef<any>(null);
  const overallTableRef = useRef<any>(null);
  const individualAssetChartRef = useRef<any>(null);
  const individualAssetTableRef = useRef<any>(null);
  const individualLiabilityChartRef = useRef<any>(null);
  const individualLiabilityTableRef = useRef<any>(null);
  const individualAssetPieChartRef = useRef<any>(null);
  const individualLiabilityPieChartRef = useRef<any>(null);
  
  const [loading, setLoading] = useState(true);
  const [projectionData, setProjectionData] = useState<FinancialRecord[] | null>(null);
  ...
  const fetchProjectionData = useCallback(async () => {
    ...
    try {
      ...
      const assetAccounts: ProjectionAccountPayload[] = assets.map((asset: FinancialRecord): ProjectionAccountPayload => {
        return {
          name: asset.name,
          account_type: 'asset',
      ...
      const liabilityAccounts: ProjectionAccountPayload[] = liabilities.map((liability: FinancialRecord): ProjectionAccountPayload => ({
        name: liability.name,
      ...
      const incomeAccounts: ProjectionAccountPayload[] = (incomeItems || []).map((income: FinancialRecord): ProjectionAccountPayload => {
      ...
      const expenseAccounts: ProjectionAccountPayload[] = (expenseItems || []).map((expense: FinancialRecord): ProjectionAccountPayload => {
  ...
  const parseProjectionData = (): any => {
    ...
    const years = projectionData.map((dp: FinancialRecord) => dp.Year || (currentYear + dp.year - 1));
    const totalAssetValues = projectionData.map((dp: FinancialRecord) => dp["Total Assets"] || 0);
    const totalLiabilityValues = projectionData.map((dp: FinancialRecord) => Math.abs(dp["Total Liabilities"] || 0)); // Convert to positive for display
    const netWorthValues = projectionData.map((dp: FinancialRecord) => dp["Net Worth"] || 0);

    // Extract individual asset projections
    const individualAssetProjections = assets.map((asset: any) => {
      const key = `${asset.name}_Value`;
      const projectedValues = projectionData.map((dp: FinancialRecord) => {
    ...
    const individualLiabilityProjections = liabilities.map((liability: any) => {
  ...
  const displayYearsIndices: number[] = [];
  ...
  // Download functions
  const handleDownloadChartPng = (chartRef: React.RefObject<any>, filename: string) => {
    if (chartRef.current) {
  ...
  const handleDownloadChartPdf = (chartRef: React.RefObject<any>, filename: string) => {
    if (chartRef.current) {
  ...
  const handleDownloadTablePdf = async (tableRef: React.RefObject<any>, filename: string) => {
    if (tableRef.current) {
  ...
  const handleDownloadCombinedProjectionCsv = (filename: string) => {
  ...
  const handleDownloadIndividualProjectionsCsv = (items: Array<{ name: string; projectedValues: number[] }>, filename: string) => {
    ...
    const headers = ['Name', ...years.map((y: any) => String(y))];
    const csvRows = [headers.join(',')];

    items.forEach((item: any) => {
      const row = [
        item.name,
        ...(isAssets ? item.projectedValues : item.projectedValues).map((val: number) => formatCurrency(val).replace(/[$,]/g, ''))
      ].map((val: any) => `"${val}"`);
      csvRows.push(row.join(','));
    });
  ...
  const prepareAssetPieData = () => {
    ...
    const labels = individualAssetProjections.map((asset: any) => asset.name);
    const data = individualAssetProjections.map((asset: any) => asset.projectedValues[lastIndex]);
    const backgroundColors = individualAssetProjections.map((asset: any, index: any) => 
      `hsl(${index * 60}, 70%, 50%)`
    );
  ...
  const prepareLiabilityPieData = () => {
    ...
    const labels = individualLiabilityProjections.map((liability: any) => liability.name);
    const data = individualLiabilityProjections.map((liability: any) => liability.projectedValues[lastIndex]);
    const backgroundColors = individualLiabilityProjections.map((liability: any, index: any) => 
  ...
  if (compact) {
    return (
      <Box className="balance-sheet-compact-chart" sx={{ ...darkChartPanelSx, p: 1.5 }}>
        <Line data={overallChartData} options={compactChartOptions as any} height={180} />
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Paper variant="outlined" sx={projectionSectionCardSx}>
        ...
        >
          <Line ref={overallChartRef} data={overallChartData} options={chartOptions as any} />
        </Box>
      </Paper>
      ...
      <Paper variant="outlined" sx={{ ...projectionSectionCardSx, mb: 4 }}>
        ...
        <TableContainer sx={{ ...projectionTableContainerSx, maxHeight: 440 }}>
          <Table stickyHeader size="small" ref={overallTableRef}>
            ...
            <TableBody>
              {displayYearsIndices.map((index: any) => (
      ...
      <Paper variant="outlined" sx={projectionSectionCardSx}>
        <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ xs: "flex-start", sm: "center" }} spacing={2} sx={{ mb: 2 }}>
          ...
          <FormControlLabel
            control={<Switch checked={showTotalsInChart} onChange={(e: any) => setShowTotalsInChart(e.target.checked)} />}
            label="Show Totals"
          />
        </Stack>
        ...
        <Box 
          ...
          <Line
            ...
            data={{
              labels: years,
              datasets: [
                ...individualAssetProjections.map((asset: any, index: any) => ({
                  label: asset.name,
      ...
      <Paper variant="outlined" sx={{ ...projectionSectionCardSx, mb: 4 }}>
        ...
        <TableContainer sx={projectionTableContainerSx}>
          <Table size="small" ref={individualAssetTableRef}>
            ...
            <TableBody>
              {individualAssetProjections.map((asset: any) => (
          ...
          >
            <Line
              ...
              data={{
                labels: years,
                datasets: [
                  ...individualLiabilityProjections.map((liability: any, index: any) => ({
        ...
        <Paper variant="outlined" sx={{ ...projectionSectionCardSx, mb: 0 }}>
          ...
          <TableContainer sx={projectionTableContainerSx}>
            <Table size="small" ref={individualLiabilityTableRef}>
              ...
              <TableBody>
                {individualLiabilityProjections.map((liability: any) => (

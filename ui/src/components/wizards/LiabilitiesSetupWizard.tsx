...
const LiabilitiesSetupWizard = ({ isOpen, onClose, onComplete }: any) => {
  ...
  const [newLiability, setNewLiability] = useState<any>({
    name: '',
    category: '',
    value: '',
  ...
  useEffect(() => {
    if (newLiability.loan_type === 'amortized') {
      ...
      if (!isNaN(principal) && !isNaN(interestRate) && !isNaN(loanTerm) && loanTerm > 0) {
        ...
        setNewLiability((prev: any) => ({ ...prev, monthly_payment: calculatedPayment.toFixed(2) }));
      } else {
        setNewLiability((prev: any) => ({ ...prev, monthly_payment: '' }));
      }
    }
  ...
  const loadData = async () => {
    try {
      ...
      if (cats.length > 0 && !newLiability.category) {
        setNewLiability((prev: any) => ({ ...prev, category: cats[0] }));
      }
    } catch (e: any) {
    }
  };

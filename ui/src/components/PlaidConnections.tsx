 ...
 */
function PlaidConnections({ onSyncSuccess }: any) {
  ...
  const handleSync = async (itemId: any) => {
    try {
      setSyncing((prev: any) => ({ ...prev, [itemId]: true }));
      setError(null);
      ...
      setSyncing((prev: any) => ({ ...prev, [itemId]: false }));
    }
  };
  ...
  const handleDisconnect = async (itemId: any) => {
    ...
    try {
      ...
      setItems((prev: any) => prev.filter((item: any) => item.item_id !== itemId));
          ...
          <div key={item.item_id} className="plaid-connection-item">
            ...
            <div className="plaid-connection-actions">
              <button
                ...
                disabled={Boolean(syncing[item.item_id])}
                style={{ fontSize: "0.9em", padding: "6px 12px" }}
              >
                {Boolean(syncing[item.item_id]) ? "Syncing..." : "Sync"}
              </button>
              <button
                className="btn-primary-modern"

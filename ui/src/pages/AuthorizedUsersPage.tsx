...
const AuthorizedUsersPage = () => {
  ...
  const [receivedAccess, setReceivedAccess] = useState<any[]>([]);
  ...
  const [editPermissions, setEditPermissions] = useState<{
    financial_data_permission?: string | null;
    document_vault_permission?: string | null;
  }>({});
  ...
  const handleAddUser = async () => {
    ...
    const hasPermission = Object.values(newPermissions).some((p: any) => p !== null);
    if (!hasPermission) {
  ...
  const handlePermissionChange = (permissionType: any, value: any, isEdit = false) => {
    if (isEdit) {
      setEditPermissions((prev: any) => ({ ...prev, [permissionType]: value === 'none' ? null : value }));
    } else {
      setNewPermissions((prev: any) => ({ ...prev, [permissionType]: value === 'none' ? null : value }));
    }
  };

  const PermissionSelector = ({ permissionType, value, onChange, label }: any) => (
    <div className="permission-selector">
      <label>{label}:</label>
      <select value={value || 'none'} onChange={(e: any) => onChange(permissionType, e.target.value)}>
  ...
  return (
    <div className="authorized-users-page">
      ...
      {!loading && !error && (
        ...
        {/* Add User Modal */}
        {showAddModal && (
          <div className="modal-overlay" onMouseDown={(e: any) => {
            if (e.target === e.currentTarget) {
              setShowAddModal(false);
            }
          }}>
            <div className="modal-content" onMouseDown={(e: any) => e.stopPropagation()}>
              ...
              <input
                ...
                onChange={(e: any) => setNewTemporaryPassword(e.target.value)}
                className="form-input"
            ...
            <div className="modal-content" onMouseDown={(e: any) => e.stopPropagation()}>
              ...
              <div className="permissions-section">
                ...
                <PermissionSelector
                  ...
                  onChange={(type: any, val: any) => handlePermissionChange(type, val, true)}
                  label="Financial Data (Accounts, Items, Projections, Charts)"
                />
                <PermissionSelector
                  permissionType="document_vault_permission"

...
const getAssetValueForYear = (asset, yearOffset) => {
...
const isDisbursementActiveInYear = (disbursement, projectionYear) => {
...
export const buildTaxableDistributionEntries = ({
  ...
  return autoDisbursements.flatMap((disbursement) => {
    ...
    const sourceAsset = assets.find(a => a.id === disbursement.source_asset_id);
    if (!sourceAsset) {
      return [];
    }

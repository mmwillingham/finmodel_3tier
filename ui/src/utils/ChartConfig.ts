...
export const generateChartData = (projectionData: any) => {
    ...
    const accountKeys = Object.keys(yearlyData).filter((key: any) => key.includes('Balance') && !key.includes('Total'));
    ...
    accountKeys.forEach((key: any, index: any) => {
    ...
    return {
        labels: years.map((y: any) => `Year ${y}`),
        datasets: datasets,
    };
};

// Function to define the Chart.js options
export const chartOptions = (finalValue: any) => ({
    responsive: true,

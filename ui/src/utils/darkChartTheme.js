...
export const formatFullCurrency = (value) => fullCurrencyFormatter.format(Number(value) || 0);
...
export const createDarkLineChartOptions = ({
  ...
  scales: {
    ...
    y: {
      ...
      ticks: {
        color: "#94a3b8",
        callback: (value) => (compactYAxis ? formatCompactCurrency(value) : formatFullCurrency(value)),
      },
      grid: {
        color: "rgba(148, 163, 184, 0.16)",
        borderColor: "rgba(148, 163, 184, 0.3)",
...
export const createDarkPieChartOptions = ({ title, legendPosition = "right", formatValue } = {}) => ({
  responsive: true,
  plugins: {
    legend: {
      position: legendPosition,
